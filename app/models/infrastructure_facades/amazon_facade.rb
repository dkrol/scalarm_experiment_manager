...
class AmazonFacade < InfrastructureFacade
  ...
  def current_state(user)
    ...
    begin
      ...
      "No information available due to excetion - #{ex}"
    end
  end
  ...
  def clean_tmp_credentials(user_id, session)
    if session.include?(:tmp_store_secrets_in_session) and (not user_id.nil?)
      AmazonSecrets.find_all_by_user_id(user_id).each do |secrets|
        secrets.destroy
      end
    end
  ...
  def get_ec2_for(user)
    ...
    ec2 = AWS::EC2.new(access_key_id: secrets.access_key, secret_access_key: secrets.secret_key)

    begin
      ec2.instances
    rescue Exception => e
      Rails.logger.debug("[aws] exception: #{e}")
      return nil
    end
